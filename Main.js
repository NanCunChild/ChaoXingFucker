...
(function (vue, pinia$1, rxjs, md5, ElementPlus) {
  ...
  const _hoisted_6 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ vue.createElementVNode("p", null, [
    /* @__PURE__ */ vue.createTextVNode(" 1.答题间隔单位为毫秒，即1000毫秒为1秒 "),
    /* @__PURE__ */ vue.createElementVNode("br"),
    /* @__PURE__ */ vue.createTextVNode(" 2.修改之后刷新页面生效 ")
  ], -1));
        ...
        vue.withDirectives(vue.createElementVNode("div", _hoisted_5, [
          vue.createVNode(_component_el_space, {
            ...
            default: vue.withCtx(() => [
              ...
              vue.createVNode(_component_el_form_item, { label: "答题间隔修改" }, {
                default: vue.withCtx(() => [
              ...
              vue.createVNode(_component_el_form_item, { label: "章节是否自动提交" }, {
                default: vue.withCtx(() => [
              ...
              vue.createVNode(_component_el_form_item, { label: "是否自动下一章节" }, {
                default: vue.withCtx(() => [
                  vue.createVNode(_component_el_switch, {
              ...
              vue.createVNode(_component_el_form_item, { label: "考试是否自动切换" }, {
                default: vue.withCtx(() => [
                  vue.createVNode(_component_el_switch, {
    ...
    constructor(iframe) {
      ...
      __publicField(this, "parseHtml", () => {
        ...
        questionElements.forEach((questionElement) => {
          ...
          let questionType;
          if (questionTypeText.match(/【单选题】/)) {
            questionType = "单选题";
          } else if (questionTypeText.match(/【填空题】/)) {
            questionType = "填空题";
          } else if (questionTypeText.match(/【判断题】/)) {
            questionType = "判断题";
          } else if (questionTypeText.match(/【简答题】/)) {
            questionType = "简答题";
          } else if (questionTypeText.match(/【多选题】/)) {
            questionType = "多选题";
          } else {
            questionType = "没有适配的题型";
          }
          const questionTitle = this.removeHtml(((_a = questionElement.querySelector(".fontLabel")) == null ? void 0 : _a.innerHTML) || "");
          questions.push({
            element: questionElement,
            type: questionType,
            title: questionTitle.replace(`【${questionType}】`, ""),
            answer: ""
          });
        });
        return questions;
      ...
      __publicField(this, "fillQuestion", (question) => {
        ...
        if (question.type === "单选题" || question.type === "多选题") {
          ...
          if (question.answer.match(/(^|,)(正确|是|对|√|T|ri|right|true)(,|$)/)) {
            (_a = question.element.querySelector('input[value="true"]')) == null ? void 0 : _a.click();
          } else if (question.answer.match(/(^|,)(错误|否|错|×|F|wr|wrong|false)(,|$)/)) {
  ...
  const _sfc_main$4 = vue.defineComponent({
    ...
    setup(props, context) {
      ...
      context.emit("send-message", `进入章节学习页面`, "primary");
      context.emit("send-message", `正在解析任务点，请稍等5-10秒（如果长时间没有反应，请刷新页面）`, "warning");
      const globalConfig = configStore();
            ...
            complete: async () => {
              if (globalConfig.chapter.autoSwitch) {
                ...
                if (nextBtn == null ? void 0 : nextBtn.getAttribute("onclick")) {
                  context.emit("send-message", `本页任务点已全部完成，正前往下一章节`, "success");
                  await sleep(3e3);
                  document.querySelector(".jb_btn.jb_btn_92.fr.fs14.nextChapter").click();
                } else
                  context.emit("send-message", `当前是最后一个任务点`, "success");
              } else {
                context.emit("send-message", `已经关闭自动下一章节，在设置里可更改`, "danger");
              }
            }
          });
        });
      ...
      const processIframe = (iframe) => {
        ...
        if (iframeSrc.includes("video")) {
          context.emit("send-message", "发现一个视频，正在解析", "warning");
          return new Promise(async (resolve) => {
            if (!iframeWindow.isUnFinishJob()) {
              context.emit("send-message", "视频已完成，跳过", "success");
              return resolve();
            }
            context.emit("send-message", "正在尝试播放，请稍等5s", "primary");
            let isExecuted = false;
            const intervalId = setInterval(async () => {
              const video = iframeDocument.documentElement.querySelector("video");
              if (video && !isExecuted) {
                context.emit("send-message", "播放成功", "success");
                if (!video)
                  return;
                await video.pause();
                video.muted = true;
                ...
                video.addEventListener("ended", () => {
                  context.emit("send-message", "视频已播放完成", "success");
          ...
          context.emit("send-message", "发现一个音频，正在解析", "warning");
          ...
          return new Promise((resolve) => {
            ...
            (async () => {
              for (const [index, question] of results.entries()) {
                context.emit("send-message", `正在搜索第${index + 1}道题`, "");
                ...
                if (answerData.code === 1) {
                  ...
                  question.answer = answerData.msg + "，可以扫描上方二维码尝试app搜题";
                }
                questionList.value.push(question);
                await sleep(globalConfig.interval);
              }
              if (globalConfig.chapter.autoSubmit) {
                context.emit("send-message", "自动提交已开启，尝试提交", "");
                await iframeWindow.btnBlueSubmit();
                await sleep(3e3);
                await iframeWindow.submitCheckTimes();
                context.emit("send-message", "提交成功", "success");
              } else {
                context.emit("send-message", "未开启自动提交，暂存", "primary");
  ...
  const _hoisted_2$2 = /* @__PURE__ */ vue.createTextVNode(" 复制 ");
    ...
    constructor() {
      ...
      __publicField(this, "fillQuestion", (question) => {
        if (question.type === "单选题" || question.type === "多选题") {
          ...
          if (question.answer.match(/(^|,)(正确|是|对|√|T|ri|right|true)(,|$)/)) {
            const choiceElement = question.element.querySelector('.answerBg[aria-label="A对选择"]');
            if (choiceElement.getAttribute("aria-checked") === "true")
              return;
            choiceElement == null ? void 0 : choiceElement.click();
          } else if (question.answer.match(/(^|,)(错误|否|错|×|F|wr|wrong|false)(,|$)/)) {
            const choiceElement = question.element.querySelector('.answerBg[aria-label="B错选择"]');
            if (choiceElement.getAttribute("aria-checked") === "true")
              return;
            choiceElement == null ? void 0 : choiceElement.click();
          }
  ...
  const _sfc_main$3 = {
    ...
    setup(props, context) {
      const globalConfig = configStore();
      context.emit("send-message", `进入新版作业页面`, "primary");
      context.emit("send-message", `正在解析题目`, "warning");
      const questionList = vue.ref([]);
      const processWorkQuestion = new Work$1();
      const results = processWorkQuestion.parseHtml();
      if (results.length) {
        context.emit("send-message", `解析到${results.length}个题目`, "success");
        (async () => {
          for (const [index, question] of results.entries()) {
            context.emit("send-message", `正在搜索第${index + 1}道题`, "");
            const answerData = await getAnswer(question.title);
        ...
        context.emit("send-message", `未解析到题目`, "red");
      return {
        questionList
      };
    }
  ...
  const _hoisted_2$1 = /* @__PURE__ */ vue.createTextVNode(" 复制 ");
  ...
  const _sfc_main$2 = vue.defineComponent({
    ...
    setup(props, context) {
      const globalConfig = configStore();
      context.emit("send-message", `进入新版考试页面`, "primary");
      const questionList = vue.ref([]);
        ...
        (async () => {
          for (const [index, question] of results.entries()) {
            ...
            if (globalConfig.exam.autoSwitch) {
              context.emit("send-message", "自动切换已开启，正在前往下一题", "success");
              await sleep(globalConfig.interval);
              document.querySelector(".jb_btn.jb_btn_92.fs14").click();
            } else {
              context.emit("send-message", "已经关闭自动切换，在设置里可更改", "danger");
            }
          }
        })();
      } else
        context.emit("send-message", `进入考试界面，本脚本自动运行`, "primary");
      return {
        questionList
      };
    }
  ...
  const _hoisted_2 = /* @__PURE__ */ vue.createTextVNode(" 复制 ");
